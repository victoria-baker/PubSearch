...
def process_articles_multithread(df, corpus, articleMap):
    ...
    target=process_article, args=(
        article, corpus, articleMap))
        thread.start()
...
def search(words):
    ...
    if X_train_counts.shape[0] > 1:
        ...
        X_filtered = np.dot(X_svd[:, important_indices], np.diag(
            singular_values[important_indices]))

        query_vector = count_vect.transform([words])
        query_svd = svd.transform(query_vector)
        query_filtered = np.dot(query_svd[:, important_indices], np.diag(
            singular_values[important_indices]))

        # end2 = time.perf_counter()
        # print(end2-start)
        similarity = cosine_similarity(
    query_filtered.reshape(
        1, -1), X_filtered)
